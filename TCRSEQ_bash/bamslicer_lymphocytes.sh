...
# just TCRB
region_19="range=chr7:142000817-142510993"
region_37="range=7:142000817-142510993"
...
function check_unmapped {
	name=$1
	idxstats=`samtools idxstats $bam_outpath/$name.unmapped.sorted.bam | head -n 1 | cut -f3`
	if [ $idxstats -ne 0 ]
		then
			mv $bam_outpath/$name.unmapped.sorted.bam $bam_outpath/$name.full.bam
			samtools view -b -f 4 $bam_outpath/$name.full.bam > $bam_outpath/$name.unmapped.sorted.bam
...
#	mv $bam_outpath/${name}.TCR.sorted.bam.bai ${base_dir}/mapped
	wait
