...
gsva_file <- ""
clinical_file <- ""
results <- matrix(NA, nrow=nrow(exome_ref), ncol=43)
for (i in 1:nrow(exome_ref))
{
  ...
  clinical_results <- rep(NA, 5)
  if (length(clinical_col>0)) {
    clinical_results <- as.matrix(clinical_file[c(3,4,5,6,7), clinical_col])
  }
...
pancan_colnames <- c("patient_uuid", "sample_barcode", "cohort", "exome_uuid", "exome_reads",
                     ...
                     "days_to_birth", "days_to_death","days_to_last_followup", "percent_tils", "gender",
...
write.table(results, "/Users/Eric/tcrseq/new/processed/pancan_exome_rna_blood_tcrb_gsva_clinical_7-29-2016_exome.txt", sep="\t", 
